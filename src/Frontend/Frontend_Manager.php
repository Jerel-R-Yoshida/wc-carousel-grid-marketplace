...
class Frontend_Manager {
    ...
    public function render_marketplace_shortcode(array $atts = []): string {
        $defaults = [
            'columns' => (int) get_option('welp_grid_columns', 3),
            'columns_tablet' => 2,
            'columns_mobile' => 1,
            'category' => '',
            'exclude_category' => '',
            'products' => '',
            'tier' => 0,
            'limit' => (int) get_option('welp_cards_per_page', 12),
            'orderby' => 'date',
            'order' => 'DESC',
            'show_sidebar' => get_option('welp_show_sidebar', true) ? 'true' : 'false',
            'show_filter' => get_option('welp_show_filter_bar', true) ? 'true' : 'false',
            'show_search' => 'false',
            'show_sort' => 'true',
            'layout' => 'grid',
            'mobile_carousel' => get_option('welp_mobile_carousel', true) ? 'true' : 'false',
            'infinite_scroll' => get_option('welp_enable_infinite_scroll', false) ? 'true' : 'false',
            'pagination' => 'numbers',
            'popular_only' => 'false',
